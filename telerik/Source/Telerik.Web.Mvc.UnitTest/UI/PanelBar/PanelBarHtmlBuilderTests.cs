...
namespace Telerik.Web.Mvc.UI.UnitTest
{
    ...
    public class PanelBarHtmlBuilderTests
    {
        ...
        {
            IHtmlNode tag = renderer.PanelBarTag();
		...
		public void Should_render_selected_class_if_item_is_selected()
		{
            ...
            IHtmlNode tag = renderer.ItemInnerTag(item.Items[0]);
        ...
        {
            IHtmlNode tag = renderer.ItemInnerTag(item);
        ...
        [Fact]
        public void Should_render_expand_arrow()
        {
            item.Enabled = true;
            item.Expanded = true;
            ...
            IHtmlNode tag = renderer.ItemInnerTag(item).Children[1];

            Assert.Equal("span", tag.TagName);
            Assert.Equal("t-icon t-arrow-up", tag.Attribute("class"));
        }
        ...
        public void Should_render_span_with_collapsed_css_class_if_expanded_property_is_false()
        {
            ...
            Assert.Equal("t-icon t-arrow-down", tag.Attribute("class"));
        }
        ...
        public void Should_output_image()
        {
            ...
            IHtmlNode tag = renderer.ItemInnerTag(item).Children[0];
        ...
        [Fact]
        public void Should_not_render_up_arrow_when_disabled() 
        {
            item.Enabled = false;
            item.ContentUrl = "#";

            IHtmlNode tag = renderer.ItemInnerTag(item).Children[1];
            Assert.Equal("t-icon", tag.Attribute("class"));
        }
    }
}
